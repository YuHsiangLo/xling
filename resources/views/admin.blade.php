                ...
                <div class="card-body">
                    ...
                    <table width="100%">
                        ...
                        @foreach ($consent_forms as $consent_form)
                            <tr><td>{{ $consent_form->consent_form_id }}</td><td>{{ $consent_form->name }}</td><td>{{ $consent_form->email }}</td><td>{{ $consent_form->created_at }}</td><td>{!! $consent_form->has_demographic_questionnaire ? '<i class="far fa-check-square" title="Questionnaire Received"></i>' : '<i class="far fa-square" title="Questionnaire NOT Received"></i>' !!}</td><td>{!! $consent_form->has_recording ? '<i class="far fa-check-square" title="Recording Received"></i>' : '<i class="far fa-square" title="Recording NOT Received"></i> ' !!}</td><td><a href="{{ route ('consent_forms.show', $consent_form->consent_form_id) }}" class="btn btn-info" title="View submission"><i class="fas fa-eye"></i></a> <a href="{{ route ('consent_forms.destroy-get', $consent_form->consent_form_id) }}" class="btn btn-info" title="Delete submission" onclick="return confirm('Are you sure you wish to delete this submission? This cannot be undone!')"><i class="fas fa-trash-alt"></i></a></td></tr>
                        @endforeach
                    </table>
                    @else
